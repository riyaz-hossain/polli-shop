...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${product.image}></img>
      ...
      <p> Total Rating : ${product.rating.count} <i class="fas fa-user"></i> </p>
      <p> Average Rating : ${product.rating.rate} <i class="fas fa-star"></i> </p>
      <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-success">add to cart</button>
      <button type="button" data-toggle="modal" data-target="#${product.id}" onclick="loadProductDetailsData(${product.id})" id="details-btn" class="btn btn-danger">Details</button></div>
      `;
...
const loadProductDetailsData = (id) => {
  fetch(`https://fakestoreapi.com/products/${id}`)
    .then(res => res.json())
    .then(data => porductDetails(data))
      ...
      <div class="modal fade" id="${product.id}" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
          <div class="modal-content">
            ...
            <img class="product-image d-block m-auto" src=${product.image}></img>
            <h5 class="text-secondary"> Price: $ ${product.price} </h5>
            <p>
            Description:  ${product.description}
            </p>
            <p className="stoke">Total Stock :  ${product.rating.count} </p>
